...
estudiantes = [{"id": 1, "nombre": "Brayan", "apellido": "Umpiri", "carrera": "Informatica"},
               ...
               {"id": 2, "nombre": "Carlos", "apellido": "Quispe", "carrera": "Ingenieria de Sistemas"}]
...
class RESTRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/lista_estudiantes":
            ...
            buscar_nombres= [estudiante["nombre"] for estudiante in estudiantes if estudiante["nombre"].startswith(cad)]
            self.send_response(200)
            self.send_header("Content-type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps({"nombres_con_{cad}": buscar_nombres}).encode("utf-8"))
